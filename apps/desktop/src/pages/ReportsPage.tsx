...
export function ReportsPage() {
  ...
  const [reportData, setReportData] = useState<ReportsPeriod | null>(null);
  ...
  const loadData = () => {
    ...
    let data: ReportsPeriod;

    switch (periodType) {
      case '7d':
        data = getReportsForLastDays(7);
        break;
      case '14d':
        data = getReportsForLastDays(14);
        break;
      case '30d':
        data = getReportsForLastDays(30);
        break;
      case 'custom':
        if (customStartDate && customEndDate) {
          data = getReportsForPeriod(customStartDate, customEndDate);
        } else {
          data = getReportsForLastDays(7);
        }
        break;
  ...
  const paymentData = useMemo(() => {
    ...
    const { cash, card, voucher } = reportData.summary.paymentMethods;
    const total = cash + card + voucher;
    if (total === 0) return [];

    return [
      { method: 'Efectivo', amount: cash, percent: (cash / total) * 100, color: '#10b981' },
      { method: 'Tarjeta', amount: card, percent: (card / total) * 100, color: '#6366f1' },
      { method: 'Voucher', amount: voucher, percent: (voucher / total) * 100, color: '#f59e0b' },
    ].filter((p) => p.amount > 0);
  }, [reportData]);
        ...
        <div className="grid grid-cols-5 gap-4">
          <div className="bg-gradient-to-br from-emerald-600/20 to-emerald-600/5 border border-emerald-600/30 rounded-xl p-4">
            ...
            <div className="flex items-center gap-1 mt-1">
              ...
              <span
                className={`text-sm ${isGrowthPositive ? 'text-emerald-400' : 'text-red-400'}`}
              >
                {formatPercent(summary.growthPercent)} vs periodo anterior
              </span>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-indigo-600/20 to-indigo-600/5 border border-indigo-600/30 rounded-xl p-4">
            ...
            <p className="text-2xl font-bold text-indigo-400">{summary.totalOrders}</p>
            <p className="text-sm text-slate-400 mt-1">
              Promedio: {formatPrice(summary.avgTicket)}
            </p>
          </div>
          ...
          <div className="bg-gradient-to-br from-purple-600/20 to-purple-600/5 border border-purple-600/30 rounded-xl p-4">
            ...
            <p className="text-2xl font-bold text-purple-400">
              {formatPrice(summary.avgDailySales)}
            </p>
            <p className="text-sm text-slate-400 mt-1">
            ...
            {summary.bestDay ? (
              <>
                <p className="text-2xl font-bold text-green-400">
                  {formatPrice(summary.bestDay.sales)}
                </p>
                <p className="text-sm text-slate-400 mt-1">
                  {formatDateDisplay(summary.bestDay.date)}
                </p>
              </>
            ) : (
            ...
            {summary.worstDay ? (
              <>
                <p className="text-2xl font-bold text-amber-400">
                  {formatPrice(summary.worstDay.sales)}
                </p>
                <p className="text-sm text-slate-400 mt-1">
                  {formatDateDisplay(summary.worstDay.date)}
                </p>
              </>
            ) : (
              <p className="text-slate-500">Sin datos</p>
            ...
            {summary.topCategories.length > 0 ? (
              <div className="space-y-3">
                {summary.topCategories.map((category) => {
                  const maxCategorySales = Math.max(...summary.topCategories.map((c) => c.sales));
